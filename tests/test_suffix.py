...
def test_suffix_different_variable_names():
    """Test suffix with various variable naming patterns."""
    model = Model()
    ...
    model.add_block(block, suffix="_s")

    # All should be suffixed independently
    assert "x_s" in model.rules["intermediate"]
    assert "x_1_s" in model.rules["intermediate"]
    assert "x_lag_s" in model.rules["intermediate"]
    assert "x_new_s" in model.rules["intermediate"]

    # x_NEXT is special: should become x_s_NEXT
    # Wait, no - x_NEXT is defined on the LHS, so it's a variable in its own right
    # So x_NEXT -> x_NEXT_s
    # But in expressions, references to x_NEXT (if x is in LHS) become x_s_NEXT
    # This is a bit confusing. Let me think...

    # Actually, the block defines a variable literally called "x_NEXT"
    # This is unusual - normally you don't define x_NEXT, you just reference it
    # But if you DO define it, it should be treated like any other variable
    # So x_NEXT (the variable) -> x_NEXT_s

    assert "x_NEXT_s" in model.rules["intermediate"]

    # log_x should reference x_s
    assert model.rules["intermediate"]["log_x_s"] == "np.log(x_s)"
